...
Подробная документация по структуре: [REFACTORING.md](./REFACTORING.md)
...
Проект настроен на автоматический деплой на GitHub Pages при пуше в ветку `main` или `master`.

### Ручной деплой

1. Соберите проект для GitHub Pages:
```bash
...
2. Настройте GitHub Pages в настройках репозитория:
   ...
   - Или используйте папку `dist` после билда

**Важно:** Убедитесь, что `VITE_BASE_PATH` в `.github/workflows/deploy.yml` соответствует имени вашего репозитория.
